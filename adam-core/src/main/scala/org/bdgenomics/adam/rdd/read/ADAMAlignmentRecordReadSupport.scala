...
class ADAMAlignmentRecordReadSupport extends DelegatingReadSupport[AlignmentRecord](new AvroReadSupport()) {
  ...
  override def prepareForRead(configuration: Configuration,
    ...
    val recordGroups = Option(keyValueMetaData)
      .map(_.asScala)
      .flatMap(_.get(recordGroupMetadata))
      .map {
        rgString =>
          jsonRead[Set[RecordGroup]](rgString)
            .map(rg => ((rg.sample, rg.recordGroupName), rg)).toMap
      }
    ...
    new RecordMaterializer[AlignmentRecord] {
      ...
      override def getCurrentRecord: AlignmentRecord = {
        val record = avroReader.getCurrentRecord
        recordGroups match {
          case Some(rg) =>
